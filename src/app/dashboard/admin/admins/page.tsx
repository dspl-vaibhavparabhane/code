...
export default function AdminAdminsPage() {
  ...
  const [filters, setFilters] = useState({ role: "Admin", name: "", email: "" });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: "Failed to fetch admin users", severity: "error" });
    } finally {
      setLoading(false);
    }
  ...
  const handleEditUser = async (id: number, payload: any) => {
    ...
    setSnackbar({ open: true, message: "Admin user updated successfully", severity: "success" });
    fetchUsers();
  };

  const handleDeleteUser = async (id: number) => {
    await usersService.deleteUser(id);
    setSnackbar({ open: true, message: "Admin user deleted successfully", severity: "success" });
    fetchUsers();
  };
      ...
      <Stack spacing={3}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Box>
            ...
            <Typography color="textSecondary">Manage all admin users</Typography>
          </Box>
          <Button
            variant="contained"
            startIcon={<AddIcon />}
